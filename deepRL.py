...
class StockEnvTrade(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            data_mean_10_days = [0]*self.stock_dim
            for i in range(self.stock_dim):
                if(len(days) == 72):
                    data_mean_10_days[i] = (days[i] + days[i+8] + days[i+16] + days[i+24] + days[i+32] +
                                            days[i+40] + days[i+48] + days[i+56] + days[i+64] + data_next_day[i]) / 10
                else:
                    data_mean_10_days[i] = data_next_day[i]

            #load next state i.e. the new value of the stocks
            self.state =  [self.state[0]] + data_mean_10_days + \
